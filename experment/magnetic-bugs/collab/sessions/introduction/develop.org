...
1. *Direction A×B*: re-run anomaly analysis /with/ path integration.
   Spatially varying bias (anomalies) breaks the same-frame cancellation
   symmetry — the bug encounters different perturbations on outbound and
   return legs. This is the natural synthesis of both directions.
2. *Corrected N_{cry} sweep*: re-run with the full ring-attractor simulation
   (slow but correct) at a few key $N_{\mathrm{cry}}$ values.
3. *CPU4 leak biology*: what are realistic leak rates? Desert ant PI
